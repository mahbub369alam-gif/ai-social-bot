...
const signToken = (id: string) => {
  ...
  return jwt.sign({ id, sellerId: id, role: "seller" }, JWT_SECRET, { expiresIn: "30d" });
};
...
const loginSeller = async (req: Request, res: Response) => {
  try {
    ...
    const rows = await queryRows<any[]>(
      "SELECT id,name,email,password_hash,is_active FROM sellers WHERE email=? LIMIT 1",
      [normEmail]
    );
    const seller = rows[0];
    ...
    const token = signToken(String(seller.id));
...
const deleteSeller = async (req: Request, res: Response) => {
  try {
    ...
    // ✅ Hard delete (UI expects it's gone), and unlock conversations
    await execResult("UPDATE conversation_locks SET seller_id=NULL, assigned_at=NULL WHERE seller_id=?", [id]);
    await execResult("DELETE FROM sellers WHERE id=?", [id]);
